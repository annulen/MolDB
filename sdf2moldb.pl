#!/usr/bin/perl
#
# sdf2moldb.pl   Norbert Haider, University of Vienna, 2009-2010
...
# based MolDB5R database. The auxiliary tables are also created, optionally 
# bitmap graphics in PNG format are created.
#
# ===========================================================================
#
...
#
# IMPORTANT: by default, all data are _appended_, no redundancy checks for 
# already existing records are made.
#
# This software was developed and tested with the SDF files from the 
# PubChem database, which are freely available at
# ftp://ftp.ncbi.nlm.nih.gov/pubchem/Compound/CURRENT-Full/SDF/
#
#
# Requirements for making bitmap graphics: "mol2ps" must be installed and 
# Ghostscript must be installed. Download site for mol2ps:
...
#$MOL2PS       = "mol2ps";    # specified in moldb5.conf
...
if ($found_mol_name == 0) {
  ...
  print "will remain empty during the SDF import operation.\n\n";
  $askuser = 1;
}
...
$createcmd = "CREATE TABLE IF NOT EXISTS $moldatatable (
  mol_id INT(11) NOT NULL DEFAULT '0',\n" ;
if ($found_mol_name == 0) {
  $createcmd = $createcmd . "  mol_name TEXT NOT NULL,\n";
}  
for ($i = 0; $i <= $#afield; $i++) {
  $l1  = $afield[$i][0];       # SDF field name
...
) ENGINE = MYISAM CHARACTER SET latin1 COLLATE latin1_swedish_ci COMMENT='Molecular data';";
$dbh->do($createcmd);
